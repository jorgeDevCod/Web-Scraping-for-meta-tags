...
document.addEventListener( 'DOMContentLoaded', () => {
    ...
    if ( copyButton ) copyButton.addEventListener( 'click', ExportarResult );
...
function getRandomMessage( stage ) {
    const messages = loaderMessages[ stage ];
    return messages[ Math.floor( Math.random() * messages.length ) ];
}

function getStatusMessage( percentage ) {
    if ( percentage === 0 ) return getRandomMessage( 'start' );
    if ( percentage === 100 ) return getRandomMessage( 'completion' );
    if ( percentage < 33 ) return `${getRandomMessage( 'processing' )} (Fase 1/3)`;
    if ( percentage < 66 ) return `${getRandomMessage( 'processing' )} (Fase 2/3)`;
    return `${getRandomMessage( 'processing' )} (Fase final)`;
}
...
function updateLoaderProgress( current, total, status = '' ) {
    ...
    if ( progressPercentage ) {
        ...
        setTimeout( () => progressPercentage.classList.remove( 'pulse' ), 300 );
    }

    // Actualizar contador de URLs con formato
    const progressStatus = document.getElementById( 'progress-status' );
    if ( progressStatus ) {
        progressStatus.textContent = `${current.toLocaleString()} / ${total.toLocaleString()}`;
    }

    // Actualizar mensaje de estado con animación
    const loaderText = document.getElementById( 'loader-text' );
    if ( loaderText ) {
        const message = status || getStatusMessage( percentage );
        if ( loaderText.textContent !== message ) {
...
function updateActionButtonsState() {
    const copyButton = document.querySelector( '.copy-button' );
    if ( copyButton ) {
        // El botón estará habilitado si hay resultados, sin importar la selección
        copyButton.disabled = allResults.length === 0;
        copyButton.classList.toggle( 'opacity-50', allResults.length === 0 );
    }
...
// Promesa que resuelve después de un tiempo determinado.
...
async function ExportarResult() {
    try {
        ...
        // Siempre exportar todos los resultados si no hay selección
        const resultsToExport = selectedRows.size > 0
            ? allResults.filter( result => selectedRows.has( result.url ) )
            : allResults;

        // Mostrar mensaje de progreso
        showNotification( `Preparando exportación de ${resultsToExport.length} registros...`, 'info' );
        ...
        ws[ '!autofilter' ] = { ref: 'A1:H1' }; // Agregar filtros

        XLSX.utils.book_append_sheet( wb, ws, 'Meta Tags' );

        // Generar nombre de archivo más descriptivo
        const date = new Date().toISOString().split( 'T' )[ 0 ];
        const selectionType = selectedRows.size > 0 ? 'selected' : 'all';
        const fileName = `meta_tags_analysis_${selectionType}_${date}.xlsx`;

        XLSX.writeFile( wb, fileName );
        showNotification( `¡Exportación exitosa! Se han exportado ${resultsToExport.length} registros`, 'success' );
    } catch ( error ) {
